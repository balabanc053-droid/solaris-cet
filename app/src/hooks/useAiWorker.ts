 ...
 * once per component mount, exposes typed send-methods, and cleans up on
 * unmount to prevent memory leaks.
 *
 * The Worker is created lazily on first use via Vite's `new Worker(new URL(…))` 
 * pattern, which causes it to be bundled as a separate async chunk and keeps
